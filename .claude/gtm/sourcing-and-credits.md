...
valid result, and only charges for what it finds. Apollo charges per record it reveals
whether the address is any good or not, which the 2026-09-02 lookup demonstrated. For a small
precise batch Clay first is probably cheaper. Not changed, because the order is Austin's call
and the Clay balance is not visible through the connection.
