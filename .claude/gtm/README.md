...
| 3. Auto-Outreach | `.claude/skills/chanty-auto-outreach/` | **Live 2026-09-12.** Sources 50 to 100 headcount in any industry and sends on its own from austin@chanty.com, 60 a day. |
| 4. Call List | `.claude/skills/chanty-call-list/` | **Live 2026-09-12.** Emails Austin 50 independent businesses to phone, weekday mornings at 7am Central. No outreach, no enrichment. |
...
staging. Same records, different methods, and `who_contacted` is what tells them apart:
`Agent 1 (automated)`, `Austin (manual)`, `Agent 3 (auto-send)`.

**Agent 3 is the one exception to the no-auto-send rule**, granted by Austin on 2026-09-11
for that agent only. Agents 1 and 2 still never send. Both follow every contract
